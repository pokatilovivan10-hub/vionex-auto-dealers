...
if (packageJson.version !== '3.10.0') errors.push(`package.json: ожидалась версия 3.10.0, получено ${packageJson.version}`);
...
if ((await read('VERSION')).trim() !== '3.10.0') errors.push('VERSION: ожидается 3.10.0.');
...
if (!seed.includes("slug: 'auto-dealers'") || !(await read('src/cms/block-definitions.mjs')).includes("titleLine1: 'Больше тест-драйвов.'") || !(await read('src/cms/block-definitions.mjs')).includes("{ title: '+50%', text: 'лидов за 14 дней'")) errors.push('v3.8.1: отсутствуют короткий оффер или ключевая инфографика автодилеров.');
