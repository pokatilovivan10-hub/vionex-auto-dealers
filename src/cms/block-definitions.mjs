...
export const BLOCK_DEFINITIONS = Object.freeze({
  ...
  'hero-auto-dealers': {
    ...
    defaults: {
      ...
      titleLine1: 'Приводим целевых лидов',
      ...
      secondaryLabel: '',
      secondaryHref: '',
      image: '/assets/img/auto-dealers-real-car.jpg',
      imageAlt: 'Автомобиль у дилерского центра',
      carX: 4,
      carY: 9,
      carScale: 108,
      sceneHeight: 560,
      badges: [
        { title: 'Вы платите только за реальных лидов!', text: '', icon: 'shield', graphic: '', x: 27, y: 78, width: 194, iconSize: 40, accent: 'violet', visualType: 'standard' },
  ...
  'auto-case-video': {
    ...
    defaults: {
      ...
      casesTitle: 'Кейсы: направления, с которыми мы работали',
      cases: [
  ...
  'pay-per-lead': {
    label: 'Оплата за лиды',
    category: 'Продажи',
    description: 'Объясняет модель оплаты за подтверждённые целевые лиды без тарифов и фиксированных цен.',
    defaults: {
      kicker: 'Прозрачная модель',
      title: 'Оплата только за подтверждённые лиды',
      intro: 'До старта согласуем, какой лид считаем целевым, как его подтверждаем и в какой момент он принимается к оплате.',
      steps: [
        { number: '01', title: 'Фиксируем критерии', text: 'Согласовываем марку, модель, географию, контактные данные и признаки целевого обращения.' },
        { number: '02', title: 'Передаём в CRM', text: 'Каждый лид приходит с источником, деталями запроса и историей обработки.' },
        { number: '03', title: 'Оплачиваете результат', text: 'В оплату входят только лиды, которые прошли согласованную проверку.' },
      ],
      note: 'Стоимость лида и правила подтверждения фиксируем до запуска в договоре.',
      buttonLabel: 'Обсудить условия',
      goal: 'pay_per_lead',
    },
    fields: [
      text('kicker', 'Надзаголовок'), text('title', 'Заголовок'), textarea('intro', 'Описание'),
      repeater('steps', 'Этапы', [text('number', 'Номер'), text('title', 'Заголовок', { required: true }), textarea('text', 'Описание')], { min: 3, max: 4 }),
      text('note', 'Уточнение'), text('buttonLabel', 'Кнопка'), text('goal', 'Цель формы'),
    ],
  },
  integrations: {
